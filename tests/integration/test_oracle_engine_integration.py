...
def oracle_target(oracle_container):
    ...
    return Target(
        name="test_oracle",
        uri=URI(f"oracle://{oracle_container['user']}:{oracle_container['password']}@"
...
class TestOracleEngineIntegration:
    ...
    def test_connection_failure_invalid_credentials(self, oracle_container, test_plan):
        ...
        target = Target(
            name="test_invalid",
            uri=URI(f"oracle://invalid:invalid@{oracle_container['host']}:{oracle_container['port']}/{oracle_container['service']}"),
            registry="invalid"
        )
    ...
    def test_registry_creation(self, oracle_engine):
        ...
        with oracle_engine.connection() as conn:
            try:
                ...
                tables = ['events', 'dependencies', 'tags', 'changes', 'projects', 'releases']
                for table in tables:
                    try:
                        conn.execute(f"DROP TABLE {oracle_engine._registry_schema}.{table} CASCADE CONSTRAINTS")
                    except:
                        pass  # Table might not exist
                
                # Drop type if it exists
                try:
                    conn.execute(f"DROP TYPE {oracle_engine._registry_schema}.sqitch_array")
                except:
    ...
    def test_registry_already_exists(self, oracle_engine):
        ...
        with oracle_engine.connection() as conn:
            conn.execute(f"SELECT COUNT(*) as count FROM {oracle_engine._registry_schema}.projects")
            result = conn.fetchone()
    ...
    def test_deploy_change(self, oracle_engine, test_change, tmp_path):
        ...
        deploy_file.write_text("CREATE TABLE test_deploy_table (id NUMBER PRIMARY KEY);")
    ...
    def test_revert_change(self, oracle_engine, test_change, tmp_path):
        ...
        deploy_dir.mkdir()
        deploy_file = deploy_dir / f"{test_change.name}.sql"
        deploy_file.write_text("CREATE TABLE test_revert_table (id NUMBER PRIMARY KEY);")
    ...
    def test_verify_change(self, oracle_engine, test_change, tmp_path):
        ...
        deploy_dir.mkdir()
        deploy_file = deploy_dir / f"{test_change.name}.sql"
        deploy_file.write_text("CREATE TABLE test_verify_table (id NUMBER PRIMARY KEY);")
    ...
    def test_get_deployed_changes(self, oracle_engine, test_change, tmp_path):
        ...
        deploy_dir.mkdir()
        deploy_file = deploy_dir / f"{test_change.name}.sql"
        deploy_file.write_text("CREATE TABLE test_deployed_table (id NUMBER PRIMARY KEY);")
    ...
    def test_sql_file_execution_with_variables(self, oracle_engine, tmp_path):
        ...
        sql_file.write_text("CREATE TABLE &table_name (id NUMBER, name VARCHAR2(&name_length));")
        
        with oracle_engine.connection() as conn:
            oracle_engine._execute_sql_file(conn, sql_file, {
                'table_name': 'test_variables_table',
                'name_length': '100'
            })
