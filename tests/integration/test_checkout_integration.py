...
"""
        ...
        subprocess.run(['git', 'add', '.'], cwd=repo_path, check=True)
        subprocess.run(['git', 'commit', '-m', 'Add posts table'], cwd=repo_path, check=True)
...
class TestCheckoutIntegration:
    ...
    def test_checkout_with_git_repo(self, checkout_command, temp_git_repo):
        ...
        try:
            ...
            with patch('sqlitch.engines.base.EngineRegistry.create_engine') as mock_create_engine:
                mock_engine = Mock()
    ...
    def test_checkout_already_on_branch(self, checkout_command, temp_git_repo):
        ...
        try:
            ...
            with patch.object(checkout_command, 'handle_error', return_value=1) as mock_handle_error:
    ...
    def test_checkout_no_common_changes(self, checkout_command, temp_git_repo):
        ...
        try:
            ...
            subprocess.run(['git', 'checkout', '-b', 'different'], cwd=temp_git_repo, check=True)
...
"""
            ...
            subprocess.run(['git', 'commit', '-m', 'Add widgets table'], cwd=temp_git_repo, check=True)
    ...
    def test_checkout_plan_parsing_error(self, checkout_command, temp_git_repo):
        ...
        try:
            ...
            subprocess.run(['git', 'checkout', '-b', 'invalid'], cwd=temp_git_repo, check=True)
            ...
            subprocess.run(['git', 'add', '.'], cwd=temp_git_repo, check=True)
            subprocess.run(['git', 'commit', '-m', 'Invalid plan'], cwd=temp_git_repo, check=True)
    ...
    def test_checkout_engine_error(self, checkout_command, temp_git_repo):
        ...
        try:
            with patch('sqlitch.engines.base.EngineRegistry.create_engine') as mock_create_engine:
                ...
                with patch.object(checkout_command, 'validate_user_info'), \
                     patch.object(checkout_command, 'handle_error', return_value=1) as mock_handle_error:
