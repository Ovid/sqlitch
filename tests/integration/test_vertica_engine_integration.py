...
from sqlitch.core.exceptions import EngineError, ConnectionError


# Skip all tests if vertica-python is not available
vertica_python = pytest.importorskip("vertica_python", reason="vertica-python not available")
...
def clean_registry(vertica_engine):
    ...
    try:
        with vertica_engine.connection() as conn:
            conn.execute(f"DROP SCHEMA IF EXISTS {vertica_engine._registry_schema_name} CASCADE")
    except Exception:
...
class TestVerticaEngineIntegration:
    ...
    def test_sql_file_execution(self, vertica_engine, clean_registry, tmp_path):
        ...
        with vertica_engine.connection() as conn:
            ...
            conn.execute(f"SELECT COUNT(*) as count FROM {vertica_engine._registry_schema_name}.test_table")
            result = conn.fetchone()
            assert result['count'] == 1
    
    def test_sql_file_execution_with_variables(self, vertica_engine, clean_registry, tmp_path):
        ...
        with vertica_engine.connection() as conn:
            vertica_engine._execute_sql_file(conn, sql_file, {'test_var': 'substituted_value'})
    ...
    def test_transaction_management(self, vertica_engine, clean_registry):
        ...
        with vertica_engine.transaction() as conn:
            ...
            """)
            conn.execute(f"INSERT INTO {vertica_engine._registry_schema_name}.transaction_test VALUES (1)")
        
        # Verify data was committed
        with vertica_engine.connection() as conn:
            conn.execute(f"SELECT COUNT(*) as count FROM {vertica_engine._registry_schema_name}.transaction_test")
            result = conn.fetchone()
            assert result['count'] == 1
        ...
        try:
            with vertica_engine.transaction() as conn:
                conn.execute(f"INSERT INTO {vertica_engine._registry_schema_name}.transaction_test VALUES (2)")
                # Force an error
    ...
    def test_regex_condition_functionality(self, vertica_engine, clean_registry):
        ...
        with vertica_engine.connection() as conn:
            ...
            """)
            conn.execute(f"INSERT INTO {vertica_engine._registry_schema_name}.regex_test VALUES ('test123')")
            conn.execute(f"INSERT INTO {vertica_engine._registry_schema_name}.regex_test VALUES ('abc456')")
            conn.execute(f"INSERT INTO {vertica_engine._registry_schema_name}.regex_test VALUES ('xyz789')")
    ...
    def test_connection_parameters_from_query_string(self, test_plan):
        ...
        target.uri.password = None
        target.uri.path = '/db'
        target.uri.query = 'user=query-user&password=query-pass&connection_load_balance=true'
        target.registry = 'sqitch'
    ...
    def test_large_sql_file_execution(self, vertica_engine, clean_registry, tmp_path):
        ...
        statements = []
        for i in range(100):
            statements.append(f"INSERT INTO {vertica_engine._registry_schema_name}.projects (project, creator_name, creator_email) VALUES ('project_{i}', 'user_{i}', 'user_{i}@example.com');")
        ...
        with vertica_engine.connection() as conn:
            ...
            conn.execute(f"SELECT COUNT(*) as count FROM {vertica_engine._registry_schema_name}.projects")
            result = conn.fetchone()
            assert result['count'] >= 100  # At least 100 (might have initial project record)
    ...
    def test_error_handling_in_sql_execution(self, vertica_engine, clean_registry, tmp_path):
