...
"""
        ...
        (self.project_dir / 'revert' / 'add_users.sql').write_text(
            "DROP TABLE users;"
        )
    ...
    def test_log_with_max_count(self, capsys):
        ...
        # Test with max count
        command = LogCommand(self.sqitch)
        result = command.execute(['--max-count', '1', '--format', 'oneline', '--no-headers'])
    ...
    def test_log_with_event_filter(self, capsys):
        ...
        command = LogCommand(self.sqitch)
        result = command.execute(['--event', 'deploy', '--format', 'oneline', '--no-headers'])
    ...
    def test_log_with_change_pattern(self, capsys):
        ...
        command = LogCommand(self.sqitch)
        result = command.execute(['--change-pattern', 'add*', '--format', 'oneline', '--no-headers'])
    ...
    def test_log_with_project_pattern(self, capsys):
        ...
        command = LogCommand(self.sqitch)
        result = command.execute(['--project-pattern', 'test*', '--format', 'oneline', '--no-headers'])
    ...
    def test_log_with_committer_pattern(self, capsys):
        ...
        command = LogCommand(self.sqitch)
        result = command.execute(['--committer-pattern', 'Test*', '--format', 'oneline', '--no-headers'])
    ...
    def test_log_with_date_format(self, capsys):
        ...
        for date_format in date_formats:
            command = LogCommand(self.sqitch)
            result = command.execute(['--date-format', date_format, '--format', 'medium', '--no-headers'])
    ...
    def test_log_with_abbrev(self, capsys):
        ...
        command = LogCommand(self.sqitch)
        result = command.execute(['--abbrev', '6', '--format', 'oneline', '--no-headers'])
