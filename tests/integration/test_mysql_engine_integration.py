...
def target(mysql_test_uri):
    ...
    return Target(
        name="mysql_test", uri=URI(mysql_test_uri), registry="sqlitch_test_registry"
    )
...
class TestMySQLEngineIntegration:
    ...
    def test_registry_version_check(self, mysql_engine):
        ...
        try:
            ...
            with mysql_engine.connection() as conn:
                conn.execute(
                    f"DROP DATABASE IF EXISTS `{mysql_engine._registry_db_name}`"
                )
    ...
    def test_change_deployment_and_revert_cycle(
        self, mysql_engine, test_change, tmp_path
    ):
    ...
    def test_change_verification(self, mysql_engine, test_change, tmp_path):
        ...
        verify_file.write_text(
            "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'verify_test';"
        )
    ...
    def test_transaction_rollback_on_error(self, mysql_engine, test_change, tmp_path):
        ...
        deploy_file = tmp_path / f"deploy_{test_change.name}.sql"
        deploy_file.write_text(
            "CREATE TABLE test_table (id INT PRIMARY KEY); INVALID SQL STATEMENT;"
        )
