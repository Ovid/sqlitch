...
"""
        ...
        # Create SQL files
        (temp_project / "deploy" / "initial.sql").write_text(
            "CREATE TABLE initial_table (id INT);"
        )
        (temp_project / "deploy" / "users.sql").write_text(
            "CREATE TABLE users (id INT, name TEXT);"
        )
        (temp_project / "deploy" / "posts.sql").write_text(
            "CREATE TABLE posts (id INT, user_id INT, title TEXT);"
        )
        (temp_project / "deploy" / "comments.sql").write_text(
            "CREATE TABLE comments (id INT, post_id INT, content TEXT);"
        )

        (temp_project / "revert" / "initial.sql").write_text(
        ...
        (temp_project / "verify" / "initial.sql").write_text(
            "SELECT 1 FROM initial_table LIMIT 1;"
        )
        (temp_project / "verify" / "users.sql").write_text(
            "SELECT 1 FROM users LIMIT 1;"
        )
        (temp_project / "verify" / "posts.sql").write_text(
            "SELECT 1 FROM posts LIMIT 1;"
        )
        (temp_project / "verify" / "comments.sql").write_text(
    ...
    def test_rebase_with_verify(self, initialized_project):
        ...
        try:
            ...
            result = command.execute(
                ["--onto-change", "initial", "--upto-change", "users", "--verify"]
            )
    ...
    def test_rebase_log_only_mode(self, initialized_project):
        ...
        try:
            ...
            result = command.execute(
                ["--onto-change", "initial", "--upto-change", "users", "--log-only"]
            )
    ...
    def test_rebase_no_prompt_mode(self, initialized_project):
        ...
        try:
            ...
            result = command.execute(
                ["--onto-change", "initial", "--upto-change", "users", "-y"]
            )

            assert result == 0
            mock_engine.revert.assert_called_once_with(
                "initial", False, True
            )  # no_prompt=False (inverted)
    ...
    def test_rebase_different_modes(self, initialized_project):
        ...
        try:
            ...
            result = command.execute(
                ["--onto-change", "initial", "--upto-change", "v1.0", "--mode", "tag"]
            )
            assert result == 0
        ...
        try:
            ...
            with patch.object(RebaseCommand, "get_engine", return_value=mock_engine):
                ...
                result = runner.invoke(
                    cli,
                    ["rebase", "--onto-change", "change1", "--upto-change", "change2"],
                )
