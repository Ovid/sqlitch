...
from sqlitch.utils.git import (
    GitRepository, detect_vcs, get_vcs_user_info, 
    is_vcs_clean, suggest_change_name, VCSError
)
...
class TestGitRepositoryIntegration:
    ...
    def test_get_status_with_staged_changes(self, git_repo_with_commits):
        ...
        subprocess.run(['git', 'add', 'test.txt'], cwd=git_repo_with_commits, check=True)
    ...
    def test_branch_operations(self, git_repo_with_commits):
        ...
        subprocess.run(['git', 'checkout', '-b', 'feature/test'], cwd=git_repo_with_commits, check=True)
...
class TestUtilityFunctionsIntegration:
    ...
    def test_suggest_change_name_feature_branch(self, git_repo_with_commits):
        ...
        # Create and switch to feature branch
        subprocess.run(['git', 'checkout', '-b', 'feature/user-auth'], cwd=git_repo_with_commits, check=True)
