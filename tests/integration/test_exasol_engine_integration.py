...
pytestmark = pytest.mark.skipif(
    not EXASOL_AVAILABLE or not all([EXASOL_TEST_DSN, EXASOL_TEST_USER, EXASOL_TEST_PASSWORD]),
...
class TestExasolEngineIntegration:
    ...
    def test_registry_creation(self, test_engine, clean_database):
        ...
        with test_engine.connection() as conn:
            ...
            for table in tables:
                conn.execute(f"SELECT COUNT(*) as count FROM {test_engine._registry_schema_name}.{table}")
                result = conn.fetchone()
    ...
    def test_sql_file_execution(self, test_engine, clean_database):
        ...
        try:
            ...
            with test_engine.connection() as conn:
                ...
                # Verify results
                conn.execute(f"SELECT COUNT(*) as count FROM {test_engine._registry_schema_name}.test_table")
                result = conn.fetchone()
                assert result['count'] == 2
                
                conn.execute(f"SELECT * FROM {test_engine._registry_schema_name}.test_table ORDER BY id")
    ...
    def test_sql_file_execution_with_variables(self, test_engine, clean_database):
        ...
        try:
            ...
            with test_engine.connection() as conn:
                ...
                # Verify results
                conn.execute(f"SELECT * FROM {test_engine._registry_schema_name}.variable_test")
                result = conn.fetchone()
                assert result['id'] == 1
    ...
    def test_transaction_management(self, test_engine, clean_database):
        ...
        # Verify table exists
        with test_engine.connection() as conn:
            conn.execute(f"SELECT COUNT(*) as count FROM {test_engine._registry_schema_name}.transaction_test")
            result = conn.fetchone()
        ...
        with test_engine.connection() as conn:
            try:
                conn.execute(f"SELECT COUNT(*) FROM {test_engine._registry_schema_name}.rollback_test")
