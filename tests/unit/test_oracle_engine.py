...
from sqlitch.engines.oracle import OracleConnection, OracleEngine, OracleRegistrySchema
    ...
    def fetchone(self):
        if self.fetchone_results:
            ...
            if isinstance(result, dict) and self.description:
                return tuple(
                    result.get(desc[0].lower(), None) for desc in self.description
                )
            return result
        return None
...
class TestOracleRegistrySchema:
    ...
    def test_get_create_statements_without_schema(self):
        ...
        assert len(statements) > 0
        assert any(
            "releases" in stmt and "testschema.releases" not in stmt
            for stmt in statements
        )
        assert any(
            "projects" in stmt and "testschema.projects" not in stmt
            for stmt in statements
        )
    ...
    def test_create_statements_include_comments(self):
        ...
        comment_statements = [
            stmt for stmt in statements if stmt.strip().startswith("COMMENT ON")
        ]
...
class TestOracleEngine:
    ...
    def test_parse_registry_schema_from_target(self, mock_cx_oracle, plan):
        ...
        target = Target(
            name="test", uri=URI("oracle://user:pass@host/db"), registry="custom_schema"
        )
    ...
    def test_parse_registry_schema_from_username(self, mock_cx_oracle, plan):
        ...
        target = Target(name="test", uri=URI("oracle://testuser:pass@host/db"))
    ...
    def test_execute_sql_file_with_variables(
        self, mock_cx_oracle, target, plan, tmp_path
    ):
    ...
    def test_create_registry(self, mock_cx_oracle, target, plan):
        ...
        assert (
            len(cursor.executed_statements) > 10
        )  # Should have many CREATE statements
    ...
    def test_get_registry_version(self, mock_cx_oracle, target, plan):
        ...
        cursor.description = [
            ("VERSION", None)
        ]  # Add description for column names (Oracle uses uppercase)
    ...
    def test_insert_release_record(self, mock_cx_oracle, target, plan):
        ...
        insert_statements = [
            stmt for stmt in cursor.executed_statements if "INSERT INTO" in stmt
        ]
    ...
    def test_insert_project_record(self, mock_cx_oracle, target, plan):
        ...
        select_statements = [
            stmt for stmt in cursor.executed_statements if "SELECT COUNT(*)" in stmt
        ]
        insert_statements = [
            stmt
            for stmt in cursor.executed_statements
            if "INSERT INTO" in stmt and "projects" in stmt
        ]
