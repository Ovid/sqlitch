...
from sqlitch.core.exceptions import SqlitchError, PlanError, EngineError
...
class TestStatusCommand:
    ...
    def test_load_plan_default(self, mock_sqitch, mock_plan):
        ...
        with patch('sqlitch.commands.status.Plan.from_file', return_value=mock_plan) as mock_from_file:
    ...
    def test_load_plan_parse_error(self, mock_sqitch):
        ...
        with patch.object(Path, 'exists', return_value=True):
            with patch('sqlitch.commands.status.Plan.from_file', side_effect=Exception("Parse error")):
    ...
    def test_emit_changes_single(self, status_command, mock_engine):
        ...
        status_command.info.assert_any_call("  test_change - 2023-01-15T10:30:00+00:00 - John Doe <john@example.com>")
    ...
    def test_emit_changes_multiple(self, status_command, mock_engine):
        ...
        status_command.info.assert_any_call("  change1            - 2023-01-15T10:30:00+00:00 - John Doe <john@example.com>")
        status_command.info.assert_any_call("  longer_change_name - 2023-01-16T11:30:00+00:00 - Jane Smith <jane@example.com>")
    ...
    def test_emit_tags_single(self, status_command, mock_engine):
        ...
        status_command.info.assert_any_call("  v1.0 - 2023-01-15T10:30:00+00:00 - John Doe <john@example.com>")
    ...
    def test_emit_status_change_not_found(self, status_command, mock_plan):
        ...
        status_command.warn.assert_called_once_with(f"Cannot find this change in {mock_plan.file}")
        status_command.error.assert_called_once_with("Make sure you are connected to the proper database for this project.")
    ...
    def test_execute_no_changes_deployed(self, status_command, mock_plan, mock_engine, mock_target):
    ...
    def test_execute_unexpected_error(self, status_command):
        ...
        assert result == 2
        status_command.error.assert_called_once_with("Unexpected error: Unexpected error")