...
from sqlitch.core.exceptions import SqlitchError, PlanError, DeploymentError
...
class TestRevertCommandPlanLoading:
    ...
    def test_load_plan_parse_error(self, revert_command):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
             patch('sqlitch.core.plan.Plan.from_file', side_effect=Exception("Parse error")):
            
            with pytest.raises(PlanError, match="Failed to load plan file.*Parse error"):
                revert_command._load_plan(plan_file)
...
class TestRevertCommandChangeDetermination:
    ...
    def test_determine_changes_all_deployed(self, revert_command, sample_plan, sample_changes):
    ...
    def test_determine_changes_to_specific_change(self, revert_command, sample_plan, sample_changes):
    ...
    def test_determine_changes_to_tag(self, revert_command, sample_plan, sample_changes):
    ...
    def test_determine_changes_log_only_mode(self, revert_command, sample_plan, sample_changes):
        """Test log-only mode."""
        mock_engine = Mock()
    ...
    def test_determine_changes_strict_mode_no_target(self, revert_command, sample_plan, sample_changes):
        """Test strict mode without target."""
        mock_engine = Mock()
        mock_engine.get_deployed_changes.return_value = [change.id for change in sample_changes]
        ...
        with pytest.raises(SqlitchError, match="Must specify a target revision in strict mode"):
            revert_command._determine_changes_to_revert(
                mock_engine, sample_plan, options
            )
    
    def test_get_changes_to_revert_to_change_not_found(self, revert_command, sample_plan, sample_changes):
    ...
    def test_get_changes_to_revert_to_change_not_deployed(self, revert_command, sample_plan, sample_changes):
    ...
    def test_get_changes_to_revert_to_tag_not_found(self, revert_command, sample_plan, sample_changes):
...
class TestRevertCommandExecution:
    ...
    def test_revert_changes_log_only(self, revert_command, sample_changes):
        ...
        with patch.object(revert_command, '_log_revert_plan', return_value=0) as mock_log:
            result = revert_command._revert_changes(mock_engine, changes_to_revert, options)
...
class TestRevertCommandIntegration:
    ...
    def test_execute_success(self, revert_command, mock_sqitch, sample_plan, sample_changes):
        ...
        with patch.object(revert_command, 'require_initialized'), \
    ...
    def test_execute_sqlitch_error(self, revert_command):
        ...
        with patch.object(revert_command, 'require_initialized', side_effect=SqlitchError("Not initialized")):
            result = revert_command.execute([])
            assert result == 1
    
    def test_execute_unexpected_error(self, revert_command):
        """Test unexpected error handling."""
        with patch.object(revert_command, 'require_initialized', side_effect=Exception("Unexpected")):
            result = revert_command.execute([])
            assert result == 2
