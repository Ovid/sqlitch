...
from sqlitch.engines.exasol import ExasolEngine, ExasolConnection, ExasolRegistrySchema
...
class TestExasolEngine:
    ...
    def setup_method(self):
        ...
        self.target = Target(
            name="test_target",
            uri=urlparse("exasol://user:pass@localhost:8563/test_db?schema=test_schema"),
    ...
    def test_parse_connection_params_defaults(self, mock_pyexasol):
        ...
        target = Target(
            name="test",
            uri=urlparse("exasol://user@host/db"),
            registry=None
        )
    ...
    def test_execute_sql_file_with_variables(self, mock_pyexasol):
        ...
        sql_content = "CREATE SCHEMA &registry; CREATE TABLE &registry.&table_name (id INT);"
    ...
    def test_create_registry_success(self, mock_pyexasol):
        ...
        assert mock_conn.execute.call_count >= 7  # Schema + 6 tables + version + project
    ...
    def test_get_deployed_changes_success(self, mock_pyexasol):
        ...
        with patch.object(engine, 'ensure_registry'):
            with patch.object(engine, 'connection') as mock_context:
                ...
                with patch.object(connection, 'fetchall', return_value=[
                    {'change_id': 'change1'},
                    {'change_id': 'change2'}
                ]):
    ...
    def test_get_deployed_changes_failure(self, mock_pyexasol):
        ...
        with patch.object(engine, 'ensure_registry'):
            with patch.object(engine, 'connection') as mock_context:
                mock_context.return_value.__enter__.return_value = ExasolConnection(mock_conn)
