...
from sqlitch.engines.mysql import MySQLConnection, MySQLEngine, MySQLRegistrySchema
...
class TestMySQLRegistrySchema:
    ...
    def test_get_create_statements(self):
        ...
        assert any(
            "CREATE TABLE IF NOT EXISTS dependencies" in stmt for stmt in statements
        )
...
class TestMySQLConnection:
    ...
    def test_execute_with_params(self, mysql_connection, mock_pymysql_conn):
        ...
        cursor.execute.assert_called_once_with(
            "SELECT * FROM table WHERE id = %(id)s", params
        )
...
class TestMySQLEngine:
    ...
    def test_parse_connection_string_with_query_params(self, plan, mock_pymysql):
        ...
        target = Target(
            name="test",
            uri=URI(
                "mysql://user:pass@localhost:3306/testdb?charset=latin1&ssl=1&connect_timeout=30"
            ),
            registry="sqitch_registry",
        )
    ...
    def test_parse_connection_string_invalid(self, mock_connect, plan, mock_pymysql):
        ...
        target = Target(
            name="test", uri=URI("invalid://connection"), registry="sqitch_registry"
        )
