...
class TestBundleCommand:
    ...
    def test_parse_args_with_dest_dir(self, bundle_command):
        ...
        options, targets, changes = bundle_command._parse_args(['--dest-dir', 'my-bundle'])
    ...
    def test_parse_args_with_from_to(self, bundle_command):
        ...
        options, targets, changes = bundle_command._parse_args(['--from', 'initial', '--to', '@v1.0'])
    ...
    def test_parse_args_with_changes(self, bundle_command):
        ...
        options, targets, changes = bundle_command._parse_args(['pg', 'initial', '@v1.0'])
    ...
    def test_validate_args_all_with_targets(self, bundle_command):
        ...
        with pytest.raises(SqlitchError, match="Cannot specify both --all and target arguments"):
    ...
    def test_validate_args_from_to_with_changes(self, bundle_command):
        ...
        with pytest.raises(SqlitchError, match="Cannot specify both --from or --to and change arguments"):
            bundle_command._validate_args(options, targets, changes)
    
    def test_validate_args_multiple_targets_with_from_to(self, bundle_command, mock_sqitch):
    ...
    def test_get_targets_to_bundle_default(self, mock_get_target, bundle_command, mock_target):
    ...
    @patch('sqlitch.commands.bundle.BundleCommand.get_target')
    def test_get_targets_to_bundle_specified(self, mock_get_target, bundle_command, mock_target):
    ...
    @patch('shutil.copy2')
    def test_copy_if_modified_new_file(self, mock_copy, bundle_command, temp_project_dir):
    ...
    def test_bundle_project_basic(self, mock_get_targets, mock_bundle_scripts, 
                                 mock_bundle_plan, mock_bundle_config, 
                                 bundle_command, mock_target, mock_sqitch):
        ...
        mock_bundle_plan.assert_called_once_with(mock_target, Path('bundle'), None, None)
        mock_bundle_scripts.assert_called_once_with(mock_target, Path('bundle'), None, None)
    ...
    @patch('sqlitch.commands.bundle.BundleCommand._get_targets_to_bundle')
    def test_execute_success(self, mock_get_targets, mock_bundle_project, 
                           bundle_command, mock_target):
