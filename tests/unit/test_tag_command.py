...
class TestTagCommand:
    ...
    def test_add_tag_success(
        self, mock_plan_from_file, mock_get_targets, tag_command, sample_plan
    ):
    ...
    @patch("sqlitch.commands.tag.TagCommand._get_targets")
    @patch("sqlitch.core.plan.Plan.from_file")
    def test_add_tag_unknown_change(
        self, mock_plan_from_file, mock_get_targets, tag_command, sample_plan
    ):
    ...
    @patch("sqlitch.commands.tag.TagCommand._get_targets")
    @patch("sqlitch.core.plan.Plan.from_file")
    def test_add_tag_no_changes(
        self, mock_plan_from_file, mock_get_targets, tag_command
    ):
        ...
        # Execute and verify
        with pytest.raises(
            SqlitchError, match='Cannot apply tag "@v1.0" to a plan with no changes'
        ):
            tag_command._add_tag("v1.0", None, {"note": []})

    @patch("sqlitch.commands.tag.TagCommand._get_targets")
    @patch("sqlitch.core.plan.Plan.from_file")
    def test_add_tag_with_change_name(
        self, mock_plan_from_file, mock_get_targets, tag_command, sample_plan
    ):
    ...
    def test_get_targets_all(self, tag_command):
        ...
        tag_command.config.get_section = Mock(
    ...
    def test_execute_add_tag(
        self, mock_add_tag, mock_validate_user, mock_require_init, tag_command
    ):
...
class TestTagCommandIntegration:
    ...
    @patch("sqlitch.commands.tag.TagCommand._get_targets")
    @patch("sqlitch.core.plan.Plan.from_file")
    def test_full_add_tag_workflow(
        self,
        mock_plan_from_file,
        mock_get_targets,
        mock_validate_user,
