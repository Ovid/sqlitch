...
class TestCheckoutCommand:
    ...
    def test_parse_args_with_options(self, checkout_command):
        ...
        args = [
            ...
            '--verify',
            '--set', 'foo=bar',
            '--set-deploy', 'deploy_var=value',
            '--set-revert', 'revert_var=other',
            '--log-only',
            '--lock-timeout', '120',
            '-y',
            'feature-branch'
    ...
    def test_parse_args_multiple_targets_warning(self, checkout_command):
        ...
        with patch.object(checkout_command, 'warn') as mock_warn:
            ...
            mock_warn.assert_called_once_with("Too many targets specified; connecting to target1")
    ...
    def test_find_last_common_change(self, checkout_command):
        ...
        # Create changes
        change1 = Change(
            name="change1", note="", timestamp=datetime.now(timezone.utc),
            planner_name="Test", planner_email="test@example.com"
        )
        change2 = Change(
            name="change2", note="", timestamp=datetime.now(timezone.utc),
            planner_name="Test", planner_email="test@example.com"
        )
        change3 = Change(
            name="change3", note="", timestamp=datetime.now(timezone.utc),
            planner_name="Test", planner_email="test@example.com"
        )
        ...
        last_common = checkout_command._find_last_common_change(current_plan, target_plan)
    ...
    def test_execute_no_branch(self, checkout_command):
        ...
        with patch.object(checkout_command, 'require_initialized'), \
    ...
    def test_execute_success(self, mock_run, checkout_command, sample_plan, sample_change):
        ...
        mock_run.side_effect = [
            ...
            Mock(stdout='current\n', returncode=0),
            # Load branch plan
            Mock(stdout='%project=test\nusers 2023-01-15T10:30:00Z John Doe <john@example.com> # Add users table', returncode=0),
            # Checkout branch
            Mock(returncode=0)
        ]
        ...
        with patch.object(checkout_command, 'require_initialized'), \
             ...
             patch('sqlitch.engines.base.EngineRegistry.create_engine') as mock_create_engine:
