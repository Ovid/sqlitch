...
from sqlitch.commands.verify import VerifyCommand, VerificationResult
...
class TestVerifyCommand:
    ...
    def test_parse_args_from_to_change(self, verify_command):
        ...
        options = verify_command._parse_args(['--from-change', 'users', '--to-change', 'posts'])
    ...
    def test_parse_args_variables(self, verify_command):
        ...
        options = verify_command._parse_args(['--set', 'key1=value1', '-s', 'key2=value2'])
    ...
    def test_load_plan_parse_error(self, verify_command):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('sqlitch.core.plan.Plan.from_file', side_effect=Exception("Parse error")):
    ...
    def test_determine_verification_range_from_change(self, verify_command, sample_plan):
    ...
    def test_determine_verification_range_change_not_deployed(self, verify_command, sample_plan):
    ...
    def test_determine_verification_range_change_not_found(self, verify_command, sample_plan):
        ...
        with patch.object(verify_command, '_parse_args', return_value={}):
            with patch.object(verify_command, '_load_plan', return_value=sample_plan):
                with patch.object(verify_command, 'get_target', return_value=Mock()):
                    with patch('sqlitch.engines.base.EngineRegistry.create_engine', return_value=mock_engine):
                        with patch.object(verify_command, '_verify_changes', return_value=0):
                            result = verify_command.execute([])
                            assert result == 0
    
    def test_execute_sqlitch_error(self, verify_command):
        """Test command execution with SqlitchError."""
        with patch.object(verify_command, '_parse_args', side_effect=SqlitchError("Test error")):
            result = verify_command.execute([])
            assert result == 1
    
    def test_execute_unexpected_error(self, verify_command):
        """Test command execution with unexpected error."""
        with patch.object(verify_command, '_parse_args', side_effect=Exception("Unexpected")):
            result = verify_command.execute([])
            assert result == 2
    ...
    def test_verify_changes_no_plan(self, verify_command, mock_engine):
        ...
        with pytest.raises(SqlitchError, match="There are deployed changes, but none planned"):
