...
class TestTemplateFunctions:
    ...
    def test_render_change_template_with_custom_dirs(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            custom_template.parent.mkdir()
            custom_template.write_text("Custom PostgreSQL deploy for {{ project }}/{{ change }}")
...
class TestTemplateIntegration:
    ...
    def test_multiple_template_directories(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            (template_dir1 / "deploy" / "custom1.tmpl").write_text("Template 1: {{ change }}")
            
            (template_dir2 / "deploy").mkdir()
            (template_dir2 / "deploy" / "custom2.tmpl").write_text("Template 2: {{ change }}")
