...
class TestDeployCommandPlanLoading:
    ...
    def test_load_plan_default(self, deploy_command, mock_sqitch):
        ...
        with (
            patch(
                "sqlitch.commands.deploy.Plan.from_file", return_value=mock_plan
            ) as mock_from_file,
            ...
            mock_from_file.assert_called_once_with(
                mock_sqitch.get_plan_file.return_value
            )
    ...
    def test_load_plan_parse_error(self, deploy_command):
        ...
        with (
            patch(
                "sqlitch.commands.deploy.Plan.from_file",
...
class TestDeployCommandChangeSelection:
    ...
    def test_determine_changes_all_pending(
        self, deploy_command, sample_plan, mock_engine
    ):
    ...
    def test_determine_changes_some_deployed(
        self, deploy_command, sample_plan, mock_engine
    ):
    ...
    def test_validate_dependencies_missing(self, deploy_command):
        ...
        changes = [
            Change(
                ...
                planner_name="Test User",
                planner_email="test@example.com",
                dependencies=[Dependency(type="require", change="missing_change")],
            )
        ]
...
class TestDeployCommandExecution:
    ...
    def test_deploy_changes_deployment_failure(
        self, deploy_command, sample_plan, mock_engine
    ):
    ...
    def test_deploy_changes_verification_failure(
        self, deploy_command, sample_plan, mock_engine
    ):
    ...
    def test_deploy_changes_keyboard_interrupt(
        self, deploy_command, sample_plan, mock_engine
    ):
...
class TestDeployCommandIntegration:
    ...
    def test_execute_success(
        self, deploy_command, mock_sqitch, sample_plan, mock_engine, mock_target
    ):
        ...
        with (
            patch.object(deploy_command, "_load_plan", return_value=sample_plan),
            patch(
    ...
    def test_execute_plan_load_error(self, deploy_command, mock_sqitch):
        ...
        with patch.object(
            deploy_command, "_load_plan", side_effect=PlanError("Plan error")
        ):
            result = deploy_command.execute([])
    ...
    def test_execute_with_verbosity(
        self, deploy_command, mock_sqitch, sample_plan, mock_engine, mock_target
    ):
