...
class TestPostgreSQLEngine:
    ...
    def test_execute_sql_file_with_variables(self, pg_engine):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            mock_conn.execute.assert_called_with("CREATE TABLE public.users (id INTEGER);")
    ...
    def test_verify_change_failure(self, pg_engine, test_change):
        ...
        with patch.object(pg_engine, 'connection') as mock_context, \
             patch.object(pg_engine, '_execute_sql_file', side_effect=Exception("Verification failed")), \
             patch('pathlib.Path.exists', return_value=True):
    ...
    def test_record_change_deployment(self, pg_engine, test_change):
        ...
        with patch.object(pg_engine, '_calculate_script_hash', return_value='hash123'), \
             patch.object(pg_engine, '_resolve_dependency_id', return_value='dep123'):
...
class TestPostgreSQLEngineIntegration:
    ...
    def test_deploy_change_with_real_sql(self, mock_read_text, mock_exists, pg_engine, test_change):
        ...
        with patch.object(pg_engine, 'transaction') as mock_transaction, \
            ...
            create_table_executed = any("CREATE TABLE users" in stmt for stmt in executed_statements)
            create_index_executed = any("CREATE INDEX idx_users_email" in stmt for stmt in executed_statements)
