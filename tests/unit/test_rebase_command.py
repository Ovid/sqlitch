...
"""
    ...
    def test_apply_config_defaults(self, rebase_command):
        ...
        rebase_command.config.get = Mock(side_effect=lambda key, as_bool=False: {
            'rebase.verify': True,
            'rebase.mode': 'change',
        ...
        assert options['no_prompt'] is True
        assert options['prompt_accept'] is True  # Should remain True since condition is not met
...
"""
    ...
    def test_determine_upto_change_explicit(self, rebase_command):
        ...
        result = rebase_command._determine_upto_change(options, engine, plan, args, onto_change)
    ...
    def test_warn_about_extra_args(self, rebase_command):
        ...
        assert 'Too many changes specified' in rebase_command.sqitch.warn.call_args[0][0]
    
    def test_collect_revert_vars(self, rebase_command):
        """Test collecting revert variables."""
        rebase_command.config.get_section = Mock(side_effect=lambda section: {
    ...
    def test_execute_success(self, rebase_command, sample_plan, tmp_path):
        ...
        with patch.object(rebase_command, '_load_plan', return_value=sample_plan):
            result = rebase_command.execute(['--onto-change', 'change1', '--upto-change', 'change2'])
    ...
    def test_execute_plan_not_found(self, rebase_command):
        ...
        with patch.object(rebase_command, '_load_plan', side_effect=PlanError("Plan not found")):
    ...
    def test_execute_not_initialized(self, rebase_command):
        ...
        rebase_command.require_initialized = Mock(side_effect=SqlitchError("Not initialized"))
    ...
    def test_execute_invalid_user_info(self, rebase_command):
        ...
        rebase_command.validate_user_info = Mock(side_effect=SqlitchError("Invalid user info"))
