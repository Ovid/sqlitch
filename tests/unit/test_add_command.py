...
class TestAddCommand:
    ...
    def test_parse_args_basic(self, add_command):
        ...
        assert options['with_scripts'] == {'deploy': True, 'revert': True, 'verify': True}
    ...
    def test_create_script_files(self, mock_template_context, mock_create_engine, 
                                add_command, mock_target):
    ...
    def test_open_editor_failure(self, mock_run, add_command):
        ...
        add_command.logger.warn.assert_called_with("Failed to open editor: Editor failed")
    
    @patch('sqlitch.commands.add.Plan.from_file')
    @patch('sqlitch.commands.add.validate_change_name')
    def test_execute_success(self, mock_validate, mock_plan_from_file, 
                           add_command, mock_target):
    ...
    def test_execute_exception(self, add_command):
        ...
        add_command.require_initialized = Mock(side_effect=SqlitchError("Not initialized"))
...
class TestAddCommandIntegration:
    ...
    def test_full_workflow(self, mock_write, mock_mkdir, mock_exists, 
                          mock_validate, mock_create_engine, mock_plan_from_file, mock_sqitch):
        ...
        command._get_targets = Mock(return_value=[mock_target])  # Return list of targets
        ...
        with patch('sqlitch.commands.add.TemplateContext') as mock_context_class:
            mock_context = Mock()
            mock_context.to_dict.return_value = {'project': 'test_project', 'change': 'test_change'}
