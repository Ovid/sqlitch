...
class TestRegistryCompatibility:
    ...
    def run_sqitch(self, args: List[str], cwd: Optional[Path] = None) -> subprocess.CompletedProcess:
    ...
    def normalize_sql_schema(self, sql: str) -> str:
        ...
        keywords = ['CREATE', 'TABLE', 'PRIMARY', 'KEY', 'NOT', 'NULL', 'UNIQUE', 'INDEX']
    ...
    def test_sqlite_registry_schema_compatibility(self):
        ...
        self.run_sqlitch(["config", "target.test.uri", f"sqlite:{sqlitch_db}"], cwd=sqlitch_dir)
        self.run_sqitch(["config", "target.test.uri", f"sqlite:{sqitch_db}"], cwd=sqitch_dir)
        
        # Deploy to create registry tables
        sqlitch_result = self.run_sqlitch(["deploy", "--target", "test"], cwd=sqlitch_dir)
        sqitch_result = self.run_sqitch(["deploy", "--target", "test"], cwd=sqitch_dir)
        
        # Both should succeed
        assert sqlitch_result.returncode == 0, f"sqlitch deploy failed: {sqlitch_result.stderr}"
        assert sqitch_result.returncode == 0, f"sqitch deploy failed: {sqitch_result.stderr}"
        ...
        assert set(sqlitch_schema.keys()) == set(sqitch_schema.keys()), \
    ...
    def test_registry_table_names_compatibility(self):
        ...
        self.run_sqlitch(["config", "target.test.uri", f"sqlite:{temp_db}"], cwd=temp_dir)
    ...
    def test_registry_initialization_idempotent(self):
        ...
        assert "Nothing to deploy" in result2.stdout or "up to date" in result2.stdout.lower()
    ...
    def test_status_output_format_compatibility(self):
        ...
        # Check status output
        sqlitch_status = self.run_sqlitch(["status", "--target", "test"], cwd=sqlitch_dir)
        ...
        assert "up to date" in sqlitch_status.stdout.lower() or "nothing to deploy" in sqlitch_status.stdout.lower()
        assert "up to date" in sqitch_status.stdout.lower() or "nothing to deploy" in sqitch_status.stdout.lower()