...
from .progress import ProgressIndicator, Spinner, ProgressBar, StatusReporter
    ...
    def complete_operation(self, success: bool = True, message: Optional[str] = None) -> None:
        ...
        if success:
            if message:
                ...
                self.sqitch.info(f"{self.operation.capitalize()} completed ({self.current_step} changes)")
            else:
...
@contextmanager
def operation_feedback(sqitch, operation: str, target: str, 
                      total_steps: int = 0) -> Iterator[OperationReporter]:
    """
...
class ChangeReporter:
    ...
    def complete_change(self, success: bool = True) -> None:
        ...
        if success:
            if hasattr(self.sqitch, 'verbosity') and self.sqitch.verbosity >= 2 and duration > 0.5:
...
def format_error_with_suggestions(error: SqlitchError, command: Optional[str] = None) -> str:
    """
    ...
    if command:
        if error.ident == 'config':
            suggestions.append(f"Try 'sqlitch help {command}' for configuration options")
            ...
            suggestions.append("Set user.name: sqlitch config --user user.name 'Your Name'")
            suggestions.append("Set user.email: sqlitch config --user user.email 'you@example.com'")
...
def show_operation_summary(sqitch, operation: str, changes: List[str], 
...
def confirm_destructive_operation(sqitch, operation: str, target: str, 
    ...
    return sqitch.ask_yes_no(f"Are you sure you want to {operation} these changes?", False)


def validate_operation_preconditions(sqitch, operation: str, 
                                   target: str) -> List[str]:
    """
