...
class Dots(ProgressIndicator):
    ...
    def __init__(
        self, message: str = "", interval: float = 0.5, max_dots: int = 3, **kwargs: Any
    ) -> None:
        """
...
class ProgressBar(ProgressIndicator):
    ...
    def __init__(
        self, total: int, message: str = "", width: int = 40, **kwargs: Any
    ) -> None:
        """
...
def show_progress(
    items: list, message: str = "Processing", show_bar: bool = True
) -> Iterator[Any]:
    """
...
def confirm_action(message: str, default: Optional[bool] = None) -> bool:
    ...
    if not sys.stdin.isatty():
        if default is not None:
            print(
                prompt
                + (getattr(i18n, "__")("Yes") if default else getattr(i18n, "__")("No"))
            )
            return default
        else:
            ...
            raise IOError(
                getattr(i18n, "__")(
                    "Sqlitch seems to be unattended and there is no default value for this question"
                )
            )
