...
class GitRepository:
    ...
    def get_status(self) -> GitStatus:
        ...
        try:
            # Get current branch
            branch_result = self._run_git_command(['rev-parse', '--abbrev-ref', 'HEAD'], check=False)
            ...
            is_clean = not has_staged_changes and not has_unstaged_changes and not untracked_files
    ...
    def get_user_name(self) -> Optional[str]:
        ...
        """
        try:
            result = self._run_git_command(['config', '--get', 'user.name'], check=False)
    ...
    def get_user_email(self) -> Optional[str]:
        ...
        """
        try:
            result = self._run_git_command(['config', '--get', 'user.email'], check=False)
            return result.stdout.strip() if result.returncode == 0 else None
        except VCSError:
            return None
    ...
    def get_file_history(self, file_path: Path, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            result = self._run_git_command([
                'log', '--oneline', f'-{limit}', '--', str(file_path)
            ], check=False)
...
def get_vcs_user_info(path: Optional[Path] = None) -> Tuple[Optional[str], Optional[str]]:
    """
...
def suggest_change_name(base_name: str, path: Optional[Path] = None) -> str:
    ...
    if status.current_branch and status.current_branch not in ['main', 'master', 'develop']:
