...
class Config:
    ...
    def _load_default_configs(self) -> None:
        ...
        for path in system_paths:
            if path.exists():
                ...
                source = ConfigSource(
                    path=path, priority=10, source_type="system", parser=parser
                )
                self._sources.append(source)
        ...
        if global_path and global_path.exists():
            ...
            source = ConfigSource(
                path=global_path, priority=20, source_type="global", parser=parser
            )
            self._sources.append(source)
        ...
        for path in local_paths:
            if path.exists():
                parser = self._load_config_file(path)
                source = ConfigSource(
                    path=path, priority=30, source_type="local", parser=parser
                )
                self._sources.append(source)
    ...
    def _get_system_config_paths(self) -> List[Path]:
        ...
        if sys.platform.startswith("win"):
            ...
            if "PROGRAMFILES" in os.environ:
                paths.append(
                    Path(os.environ["PROGRAMFILES"]) / "Sqlitch" / "etc" / "sqitch.conf"
                )
        else:
    ...
    def _load_config_file(self, config_path: Path) -> configparser.ConfigParser:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Cannot read configuration file: {e}", config_file=str(config_path)
            )
        except configparser.Error as e:
            raise ConfigurationError(
                f"Invalid configuration syntax: {e}", config_file=str(config_path)
            )
    ...
    def _merge_parser_into_dict(
        self, parser: configparser.ConfigParser, target: Dict[str, Any]
    ) -> None:
    ...
    def get_target(self, name: str) -> Target:  # noqa: C901
        ...
        if not target_config:
            ...
            if name == "default":
                ...
                engine_config = self._get_nested_value(
                    self._merged_config, f"engine.{engine}"
                )
                if not engine_config:
                    raise ConfigurationError(
                        f"No configuration found for engine '{engine}'"
                    )

                uri = engine_config.get("target")
                if not uri:
                    raise ConfigurationError(
                        f"No target URI configured for engine '{engine}'"
                    )
        ...
        deploy_dir = Path(
            target_config.get("deploy_dir", self.get("core.deploy_dir", "deploy"))
        )
        revert_dir = Path(
            target_config.get("revert_dir", self.get("core.revert_dir", "revert"))
        )
        verify_dir = Path(
            target_config.get("verify_dir", self.get("core.verify_dir", "verify"))
        )
        plan_file = Path(
            target_config.get("plan_file", self.get("core.plan_file", "sqitch.plan"))
        )
    ...
    def get_engine_config(self, engine_type: EngineType) -> Dict[str, Any]:
        ...
        """
        engine_config = self._get_nested_value(
            self._merged_config, f"engine.{engine_type}"
        )
        ...
        for target_name in self.list_targets():
            try:
                ...
                if not validate_uri(target.uri):
                    issues.append(
                        f"Invalid URI for target '{target_name}': {target.uri}"
                    )
            except ConfigurationError as e:
                issues.append(str(e))
