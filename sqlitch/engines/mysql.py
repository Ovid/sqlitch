...
class MySQLEngine(Engine):
    ...
    def __init__(self, target: Target, plan: Plan) -> None:
        ...
        self._registry_db_name = target.registry or self._connection_params.get(
            "database", "sqitch"
        )
                ...
                for key, values in query_params.items():
                    if values:
                        ...
                        if key == "charset":
                            ...
                            params["ssl"] = (
                                {"ssl": True}
                                if values[0].lower() in ("1", "true", "yes")
                                else None
                            )
                        elif key == "ssl_ca":
    ...
    def _create_connection(self) -> MySQLConnection:
        ...
        try:
            self.logger.debug(
                f"Connecting to MySQL: {sanitize_connection_string(str(self.target.uri))}"
            )
    ...
    def _create_registry(self, connection: MySQLConnection) -> None:
        ...
        try:
            ...
            connection.execute(
                f"CREATE DATABASE IF NOT EXISTS `{self._registry_db_name}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
            )
            ...
            raise EngineError(
                f"Failed to create MySQL registry: {e}", engine_name="mysql"
            ) from e
    ...
    def get_deployed_changes(self) -> List[str]:
        ...
        with self.connection() as conn:
            try:
                ...
                raise EngineError(
                    f"Failed to get deployed changes: {e}", engine_name="mysql"
                ) from e

    def _record_change_deployment(
        self, connection: MySQLConnection, change: Change
    ) -> None:
        """
    ...
    def transaction(self) -> Iterator[MySQLConnection]:
        ...
        with self.connection() as conn:
            try:
                ...
                raise DeploymentError(
                    f"Transaction failed: {e}", engine_name="mysql"
                ) from e
            finally:
                try:
