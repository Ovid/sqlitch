...
class RevertCommand(BaseCommand):
    ...
    def execute(self, args: List[str]) -> int:
        ...
        try:
            ...
            changes_to_revert = self._determine_changes_to_revert(engine, plan, options)
    ...
    def _determine_changes_to_revert(
    ...
    def _get_changes_to_revert_to_change(
        self, plan: Plan, deployed_changes: List[Change], target_change: str
    ) -> List[Change]:
        """
    ...
    def _get_changes_to_revert_to_tag(
        self, plan: Plan, deployed_changes: List[Change], target_tag: str
    ) -> List[Change]:
        """
    ...
    def _revert_changes(
        ...
        self.info(
            f"Reverting {total_changes} change{'s' if total_changes != 1 else ''}"
        )
        ...
        try:
            for i, change in enumerate(changes, 1):
                ...
                try:
                    ...
                    if reverted_count > 0:
                        self.info(
                            f"Successfully reverted {reverted_count} change{'s' if reverted_count != 1 else ''}"
                        )
            ...
            if reverted_count > 0:
                self.info(
                    f"Successfully reverted {reverted_count} change{'s' if reverted_count != 1 else ''} before cancellation"
                )
            return 130
    ...
    def _log_revert_plan(self, changes: List[Change]) -> int:
        ...
        self.info(
            f"Would revert {len(changes)} change{'s' if len(changes) != 1 else ''}:"
        )
...
@click.option(
    "--log-only", is_flag=True, help="Show what would be reverted without executing"
)
...
@click.option(
    "-m", "--modified", is_flag=True, help="Revert to common ancestor with VCS"
)
