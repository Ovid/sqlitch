...
class DeployCommand(BaseCommand):
    ...
    def execute(self, args: List[str]) -> int:
        ...
        try:
            ...
            changes_to_deploy = self._determine_changes_to_deploy(
                engine, plan, options
            )
    ...
    def _determine_changes_to_deploy(self, engine, plan: Plan, 
        ...
        if options.get('to_change'):
            ...
            pending_changes = [
                ...
                if change.id in target_change_ids
            ]
    ...
    def _validate_dependencies(self, changes: List[Change], 
                             deployed_change_ids: Set[str]) -> None:
        """
        ...
        change_name_to_id = {}
        if all_plan_changes:
            change_name_to_id = {change.name: change.id for change in all_plan_changes.changes}
    ...
    def _deploy_changes(self, engine, changes: List[Change], 
        ...
        self.info(f"Deploying {total_changes} change{'s' if total_changes != 1 else ''}")
        ...
        try:
            for i, change in enumerate(changes, 1):
                ...
                try:
                    ...
                    if deployed_count > 0:
                        self.info(f"Successfully deployed {deployed_count} change{'s' if deployed_count != 1 else ''}")
            ...
            if deployed_count > 0:
                self.info(f"Successfully deployed {deployed_count} change{'s' if deployed_count != 1 else ''} before cancellation")
            return 130
    ...
    def _log_deployment_plan(self, changes: List[Change]) -> int:
        ...
        self.info(f"Would deploy {len(changes)} change{'s' if len(changes) != 1 else ''}:")
...
@click.option('--verify/--no-verify', default=True, help='Verify each change after deployment')
@click.option('--log-only', is_flag=True, help='Show what would be deployed without executing')
