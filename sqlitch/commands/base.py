...
class BaseCommand(ABC):
    ...
    def handle_error(self, error: Exception, context: Optional[str] = None) -> int:
        ...
        Returns:
            Exit code
        """
        from ..core.exceptions import handle_exception, SqlitchError
        ...
        if isinstance(error, SqlitchError):
            ...
            formatted_error = format_error_with_suggestions(error, self.__class__.__name__.lower().replace('command', ''))
            
            # Use enhanced format for vent (stderr)
            self.vent(formatted_error)
            
            # Use simple format for error method (backward compatibility)
            simple_error = f"sqlitch: {error.message}" if not error.message.startswith("sqlitch:") else error.message
    ...
    def validate_preconditions(self, operation: str, target: Optional[str] = None) -> None:
        """
        ...
        errors = validate_operation_preconditions(self.sqitch, operation, target or "default")
        if errors:
            raise SqlitchError("\n".join(errors))
    
    def confirm_destructive_operation(self, operation: str, target: str, 
