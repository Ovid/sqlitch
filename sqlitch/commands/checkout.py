...
class CheckoutCommand(BaseCommand):
    ...
    def execute(self, args: List[str]) -> int:
        ...
        try:
            ...
            last_common_change = self._find_last_common_change(current_plan, target_plan)
            ...
            self.info(f"Last change before the branches diverged: {last_common_change.format_name_with_tags()}")
            ...
            if arg in ['--help', '-h']:
                ...
                if not options['branch']:
                    ...
                    if not options['target']:
                        ...
                        self.warn(f"Too many targets specified; connecting to {options['target']}")
    ...
    def _get_config_defaults(self) -> Dict[str, Any]:
        ...
        if self.config.get('checkout.prompt_accept') is not None:
            defaults['prompt_accept'] = self.config.get('checkout.prompt_accept', as_bool=True)
        elif self.config.get('revert.prompt_accept') is not None:
            defaults['prompt_accept'] = self.config.get('revert.prompt_accept', as_bool=True)
        
        # Check for strict mode
        if (self.config.get('checkout.strict', as_bool=True) or 
            self.config.get('revert.strict', as_bool=True)):
            raise SqlitchError(
    ...
    def _load_branch_plan(self, branch: str, target) -> Plan:
        ...
        plan_file = target.plan_file or Path(self.config.get('core.plan_file', 'sqitch.plan'))
    ...
    def _find_last_common_change(self, current_plan: Plan, target_plan: Plan) -> Optional[Change]:
    ...
    def _revert_to_common_change(self, engine, last_common_change: Change, options: Dict[str, Any]) -> None:
