...
from ..core.exceptions import SqlitchError, PlanError, EngineError
...
class StatusCommand(BaseCommand):
    ...
    def _get_current_state(self, engine, project: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def _emit_changes(self, engine, project: Optional[str], options: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            max_name_len = max(len(change['change']) for change in changes) if changes else 0
            ...
            for change in changes:
                ...
                self.info(f"  {change['change']}{name_padding} - {date_str} - {change['committer_name']} <{change['committer_email']}>")
                
        except Exception as e:
            self.warn(f"Failed to get deployed changes: {e}")
    
    def _emit_tags(self, engine, project: Optional[str], options: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            for tag in tags:
                ...
                self.info(f"  {tag['tag']}{name_padding} - {date_str} - {tag['committer_name']} <{tag['committer_email']}>")
                
        except Exception as e:
            self.warn(f"Failed to get deployed tags: {e}")
    
    def _emit_status(self, state: Dict[str, Any], plan: Plan, options: Dict[str, Any]) -> None:
        """
        ...
        if current_index is None:
            ...
            self.error("Make sure you are connected to the proper database for this project.")
            return
