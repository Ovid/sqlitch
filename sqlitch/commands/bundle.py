...
class BundleCommand(BaseCommand):
    ...
    def _looks_like_change_spec(self, arg: str) -> bool:
        ...
        return (
            arg.startswith("@")
            or ":" in arg
            or arg in ("HEAD", "ROOT", "@HEAD", "@ROOT")
        )

    def _validate_args(
        self, options: Dict[str, Any], targets: List[str], changes: List[str]
    ) -> None:
        """
    ...
    def _get_targets_to_bundle(
        self, target_names: List[str], options: Dict[str, Any]
    ) -> List[Target]:
        """
    ...
    def _bundle_project(
        self, targets: List[Target], changes: List[str], options: Dict[str, Any]
    ) -> int:
        """
        Bundle the project.
    ...
    def _dest_dirs_for(self, target: Target, dest_dir: Path) -> Dict[str, Path]:
        ...
        return {
            ...
            "reworked_deploy": base_dest
            / target.deploy_dir,  # TODO: Handle reworked dirs
