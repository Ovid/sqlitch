...
class RebaseCommand(BaseCommand):
    ...
    def execute(self, args: List[str]) -> int:
        ...
        try:
            ...
            onto_change = self._determine_onto_change(options, engine, plan, args)
    ...
    def _apply_config_defaults(self, options: Dict[str, Any]) -> None:
        ...
        if options.get("verify") is None:
            options["verify"] = (
                config.get("rebase.verify", as_bool=True)
                or config.get("deploy.verify", as_bool=True)
                or False
            )

        if options.get("mode") == "all":
            options["mode"] = (
                config.get("rebase.mode") or config.get("deploy.mode") or "all"
            )

        if options.get("no_prompt") is False:
            options["no_prompt"] = (
                config.get("rebase.no_prompt", as_bool=True)
                or config.get("revert.no_prompt", as_bool=True)
                or False
            )

        if options.get("prompt_accept") is True:
            options["prompt_accept"] = (
                config.get("rebase.prompt_accept", as_bool=True)
                or config.get("revert.prompt_accept", as_bool=True)
                or True
            )

        # Check for strict mode
        strict_mode = (
            config.get("rebase.strict", as_bool=True)
            or config.get("revert.strict", as_bool=True)
            or False
        )
    ...
    def _determine_onto_change(
        self, options: Dict[str, Any], engine, plan: Plan, args: List[str]
    ) -> Optional[str]:
    ...
    def _warn_about_extra_args(
        self,
        options: Dict[str, Any],
        args: List[str],
        onto_change: Optional[str],
        upto_change: Optional[str],
    ) -> None:
    ...
    def _execute_rebase(
        ...
        try:
            if hasattr(engine, "revert"):
                engine.revert(
                    onto_change, not options["no_prompt"], options["prompt_accept"]
                )
            else:
    ...
    def _manual_revert(
        self, engine, onto_change: Optional[str], options: Dict[str, Any]
    ) -> None:
    ...
    def _manual_deploy(
        self, engine, upto_change: Optional[str], options: Dict[str, Any]
    ) -> None:
...
@click.option(
    "--verify/--no-verify", default=None, help="Verify changes after deployment"
)
...
@click.option(
    "-y", "--yes", "no_prompt", is_flag=True, help="Do not prompt for confirmation"
)
@click.option(
    "--mode",
    type=click.Choice(["change", "tag", "all"]),
    default="all",
