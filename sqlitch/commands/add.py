...
from ..utils.template import create_template_engine, TemplateContext
...
class AddCommand(BaseCommand):
    ...
    def execute(self, args: List[str]) -> int:
        ...
        try:
            ...
            for target in targets:
                ...
                self.logger.emit(f"Added '{change.format_name_with_tags()}' to {plan.file}")
    ...
    def _parse_args(self, args: List[str]) -> Tuple[Optional[str], Dict[str, Any]]:
        """
    ...
    def _create_script_files(self, change: Change, target: Any, options: Dict[str, Any]) -> List[Path]:
        """
        ...
        context = TemplateContext(
            ...
            requires=[dep.change for dep in change.dependencies if dep.type == 'require'],
            conflicts=[dep.change for dep in change.dependencies if dep.type == 'conflict']
        )
...
@click.option('--template-directory', type=click.Path(exists=True, file_okay=False, path_type=Path), help='Template directory')
@click.option('--with', 'with_scripts', multiple=True, type=click.Choice(['deploy', 'revert', 'verify']), help='Include script type')
@click.option('--without', 'without_scripts', multiple=True, type=click.Choice(['deploy', 'revert', 'verify']), help='Exclude script type')
@click.option('-s', '--set', 'variables', multiple=True, help='Set template variable (key=value)')
@click.option('-e', '--edit', '--open-editor', is_flag=True, help='Open files in editor')
@click.pass_context
def add_command(ctx: click.Context, change_name: Optional[str], **kwargs) -> None:
    """Add a new change to sqlitch plans."""
