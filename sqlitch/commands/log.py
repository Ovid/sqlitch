...
from ..core.exceptions import SqlitchError, EngineError
from ..utils.formatter import ItemFormatter, FORMATS
        ...
        while i < len(args):
            ...
            if arg in ('-t', '--target'):
                ...
                try:
                    ...
                    raise SqlitchError(f"Invalid max-count value: {arg.split('=', 1)[1]}")
                i += 1
            elif arg == '--skip':
                if i + 1 >= len(args):
                ...
                i += 2
            elif arg.startswith('--committer-pattern=') or arg.startswith('--committer='):
...
@click.option('--event', multiple=True, help='Event type to filter by (can be used multiple times)')
@click.option('--change-pattern', '--change', help='Regular expression to match change names')
@click.option('--project-pattern', '--project', help='Regular expression to match project names')
@click.option('--committer-pattern', '--committer', help='Regular expression to match committer names')
@click.option('--planner-pattern', '--planner', help='Regular expression to match planner names')
@click.option('--format', '-f', default='medium', help='Output format (raw, full, long, medium, short, oneline)')
@click.option('--date-format', '--date', default='iso', help='Date format (iso, raw, short, or strftime format)')
@click.option('--max-count', '-n', type=int, help='Maximum number of entries to display')
@click.option('--skip', type=int, help='Number of entries to skip')
@click.option('--reverse/--no-reverse', default=False, help='Reverse the order of entries')
@click.option('--color', type=click.Choice(['always', 'never', 'auto']), default='auto', help='When to use color')
@click.option('--no-color', is_flag=True, help='Never use color (same as --color=never)')
@click.option('--abbrev', type=int, default=0, help='Abbreviate change IDs to this many characters')
@click.option('--oneline', is_flag=True, help='Shorthand for --format=oneline --abbrev=6')
