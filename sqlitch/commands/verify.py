...
class VerificationResult:
    ...
    def __init__(self, change: Change, success: bool, error: Optional[str] = None,
                 out_of_order: bool = False, not_in_plan: bool = False,
                 not_deployed: bool = False, reworked: bool = False):
        self.change = change
    ...
    def has_errors(self) -> bool:
        ...
        return not self.success or self.out_of_order or self.not_in_plan or self.not_deployed
...
class VerifyCommand(BaseCommand):
    ...
    def _determine_verification_range(self, plan: Plan, deployed_changes: List[Change],
        ...
        if options.get('from_change'):
            ...
            for i, change in enumerate(deployed_changes):
                ...
                if any(c.name == from_change or c.id == from_change for c in plan.changes):
                    raise SqlitchError(f'Change "{from_change}" has not been deployed')
                else:
                    raise SqlitchError(f'Cannot find "{from_change}" in the database or the plan')
        ...
        if options.get('to_change'):
            ...
            for i, change in enumerate(deployed_changes):
                ...
                if any(c.name == to_change or c.id == to_change for c in plan.changes):
                    ...
                    raise SqlitchError(f'Cannot find "{to_change}" in the database or the plan')
        
        return from_idx, to_idx
    
    def _run_verifications(self, engine, plan: Plan, changes: List[Change],
                          options: Dict[str, Any]) -> List[VerificationResult]:
        """
    ...
    def _run_sequential_verifications(self, engine, plan: Plan, changes: List[Change],
                                    options: Dict[str, Any], max_name_len: int) -> List[VerificationResult]:
        """
    ...
    def _run_parallel_verifications(self, engine, plan: Plan, changes: List[Change],
                                   options: Dict[str, Any], max_name_len: int) -> List[VerificationResult]:
        """
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_index = {
                executor.submit(self._verify_single_change, engine, plan, change, i, options): i
                for i, change in enumerate(changes)
            }
    ...
    def _verify_single_change(self, engine, plan: Plan, change: Change,
                             expected_index: int, options: Dict[str, Any]) -> VerificationResult:
        """
    ...
    def _emit_verification_result(self, result: VerificationResult, max_name_len: int) -> None:
        """
    ...
    def _check_undeployed_changes(self, plan: Plan, deployed_change_ids: Set[str],
                                from_idx: int, to_idx: int) -> List[VerificationResult]:
        """
    ...
    def _report_results(self, results: List[VerificationResult], plan: Plan,
                       from_idx: int, to_idx: int, options: Dict[str, Any]) -> int:
        """
        ...
        if error_count > 0:
            ...
            if to_idx < len(plan.changes) - 1:
                ...
                if pending_changes:
                    ...
                    label = "Undeployed change:" if count == 1 else "Undeployed changes:"
                    self.info(label)
...
@click.option('--from-change', '--from', help='Verify changes from this change (inclusive)')
@click.option('--to-change', '--to', help='Verify changes up to this change (inclusive)')
@click.option('--set', '-s', 'variables', multiple=True, help='Set variable for verification scripts (key=value)')
@click.option('--parallel/--no-parallel', default=True, help='Run verifications in parallel')
@click.option('--max-workers', type=int, help='Maximum number of parallel workers')
@click.pass_context
def verify_command(ctx: click.Context, from_change: Optional[str], to_change: Optional[str], **kwargs) -> None:
    """Verify deployed database changes by running verification scripts."""
