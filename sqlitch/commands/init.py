...
class InitCommand(BaseCommand):
    ...
    def _validate_project_name(self, project_name: str) -> None:
        ...
        """
        if not project_name:
            raise SqlitchError('Invalid project name "": project names must not be empty')
        ...
        if any(char in project_name for char in '@:#[] '):
            raise SqlitchError(f'Invalid project name "{project_name}": contains prohibited characters')
    ...
    def _write_plan(self, project_name: str, options: Dict[str, Any]) -> None:
        ...
        if plan_file.exists():
            self.logger.info(f"Plan file {plan_file} already exists for project '{project_name}'")
            return
